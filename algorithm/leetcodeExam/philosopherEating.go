...
func (p *DiningPhilosophers) WantToEat(philosopher int, pickLeftFork func(int), pickRightFork func(int), eat func(int), putLeftFork func(int), putRightFork func(int)) {
	defer p.wg.Done()
	...
	for {
		select {
		case p.streamForks[leftNum] <- philosopher: //嘗試拿起左邊叉子
			PickLeftFork(philosopher) //成功拿起左邊叉子
			select {
			case p.streamForks[rightNum] <- philosopher: //嘗試拿起右邊叉子
				PickRightFork(philosopher) //成功拿起又邊叉子
				Eat(philosopher)           //左右邊都拿到了，開始吃
				<-p.streamForks[leftNum]   //吃完了，放下左邊叉子
				PutLeftFork(philosopher)
				<-p.streamForks[rightNum] //吃完了，放下右邊叉子
				PutRightFork(philosopher)
				return //吃飽離開
			default: //無法拿起右邊叉子
				fmt.Printf("Philosopher %d can't pick fork %d.\n", philosopher, rightNum)
				<-p.streamForks[leftNum] //把已經拿起來的左邊叉子釋放出去
				PutLeftFork(philosopher)
			}
		default: //無法拿起左邊叉子
			fmt.Printf("Philosopher %d can't pick fork %d.\n", philosopher, leftNum)
		}
		p.missingDoubleForkTimes++
		Think()
	}
}
